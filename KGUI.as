...
class Menu : GenericGUIItem{
	
	Button[] Buttons;
	Button[] Buttons2; // this is me being lazy
	bool page2 = false;
	...
	u8 Padding;
	//In constructor you can setup any additional inner UI elements
	Menu(Vec2f _tl, Vec2f _br, Vec2f _buttonsize, u8 _padding)
	{
		position = _tl;
		size = _br;
	...
	void addScrollX(u16 buttonCount)
	{
		ScrollBarHor _scrollbar(Vec2f(position.x, size.y), Vec2f(size.x, size.y+32), buttonCount);
		if (_scrollbar !is null)
		scroll = _scrollbar;
	}

	void addTextButton(Vec2f _position, string _desc, SColor _color)
	{
		Button button(	position+Vec2f(Padding+(_position.x*(Padding+ButtonSize.x)), Padding+(_position.y*(Padding+ButtonSize.y))), //pos
						ButtonSize, _desc, _color );
		Buttons.push_back(button);
	}	

	void addColorButton(Vec2f _position, SColor _color)
	{
		Button button(	position+Vec2f(Padding+(_position.x*(Padding+ButtonSize.x)), Padding+(_position.y*(Padding+ButtonSize.y))), ButtonSize, "", _color );
		Buttons.push_back(button);
	}	

	void addIconButton(Vec2f _position, string _iconname, float _iconscale, Vec2f _iconOffset)
	{
		Button button(	position+Vec2f(Padding+(_position.x*(Padding+ButtonSize.x)), Padding+(_position.y*(Padding+ButtonSize.y))), //pos
						ButtonSize, _iconOffset, _iconname, _iconscale );
		Buttons.push_back(button);
	}	

	void addIconButton2(Vec2f _position, string _iconname, float _iconscale, Vec2f _iconOffset)
	{
		Button button(	position+Vec2f(Padding+(_position.x*(Padding+ButtonSize.x)), Padding+(_position.y*(Padding+ButtonSize.y))), //pos
						ButtonSize, _iconOffset, _iconname, _iconscale );
	...
	//Override this method to draw object. You can rely on position and size here.
	void drawSelf(){
		GUI::DrawFramedPane(position, size);
		GenericGUIItem::drawSelf();
		...
		if (!page2)
		{
			for (int i = 0; i < Buttons.length(); i++)
			{
				...
				if (btn !is null)
				{
					if (btn.position.x+btn.size.x < size.x && btn.position.y+btn.size.y < size.y && btn.position.x > position.x && btn.position.y > position.y)
					btn.draw();
				}
			}
		}	
...
class ScrollBarHor : GenericGUIItem{
	...
	ScrollBarHor(Vec2f _tl, Vec2f _br, u16 _buttoncount){

		position = _tl;
		size = _br;

		//Button _leftbutt( position+Vec2f(2,2), Vec2f(28,28), Vec2f(6,6), "$left_arrow$", 1.0f );
		//ButtonL = _leftbutt;
		//Button _rightbutt( position+Vec2f(size.x-48,2), Vec2f(28,28), Vec2f(6,6), "$right_arrow$", 1.0f );
		//ButtonR = _rightbutt;
		Button _handlebutt( position+Vec2f(0,2), Vec2f(28,28));//
		_handlebutt.isDragable = true;
		Handle = _handlebutt;
...
class Button : GenericGUIItem{
	...
	Button(Vec2f _position, Vec2f _size, string _desc){
		super(_position,_size);
		desc = _desc;
		selfLabeled = true;		
	}
	...
	Button(Vec2f _position, Vec2f _size, Vec2f _iconOffset, string _iconname, float _iconscale){
		super(_position,_size);

		IconOffset = _iconOffset;
		Icon i(_iconname, _position+_iconOffset, _iconscale);
		icon = i;
	}
	...
	void drawSelf(){
		...
		if (color != color_white) // not not implemented
		GUI::DrawRectangle(position+Vec2f(8,8), position+size-Vec2f(8,8), color);

		GenericGUIItem::drawSelf();

		if (icon !is null && icon.name != "")
		icon.drawSelf();
	}
}
...
class Icon : GenericGUIItem{
	...
	//Animated Icon setup
	Icon(string image, Vec2f _position, float _scale){
		position = _position;
		name = image;	
		scale = _scale;
	}
